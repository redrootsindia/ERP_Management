...
const MenuLeft = ({
  ...
  const applySelectedKeys = () => {
    ...
    const selectedItem = find(flattenItems(menuData, 'children'), ({ url }) =>
  ...
  const generateMenuItems = () => {
      ...
      if (item.category) {
        return (
          <Menu.ItemGroup
            ...
            title={
              <div key={item.key}>
                {counter === 0 ? null : <Divider className="menu-category-divder" />}
                {item.title}
              </div>
            }
          />
    ...
    const generateSubmenu = (items) =>
      items.map((menuItem) => {
        const { permissionsArray } = menuItem
        if (permissionsArray && !intersection(permissionsArray, permissions).length) return null
      ...
      >
        <div className={style.logoContainer}>
          <div className={style.logo}>
            <img src="resources/images/logo/logo-mark.png" alt="logo" width="40px" />
            <div className={style.name}>
              &nbsp;
              <img src="resources/images/logo/logo-text-white.png" alt="logo" />
            </div>
          </div>
        </div>
        <PerfectScrollbar>
