...
import { PRINTER_PRODUCT_PURCHASE_ORDER_PDF } from '../queries'

const PDF = ({ id }) => {
  const [pdfData, setPDFData] = useState(null)
  const [buttonClicked, setButtonClicked] = useState(false)

  const [getPDFData, { loading, data, error }] = useLazyQuery(PRINTER_PRODUCT_PURCHASE_ORDER_PDF, {
    variables: { id },
  })

  useEffect(() => {
    if (data && data.printerProductPOPDFData) {
      const { terms_conditions } = data.printerProductPOPDFData

      const tempData = {
        ...data.printerProductPOPDFData,
        termsConditions:
          terms_conditions ||
      ...
      setPDFData(tempData)
    } else if (buttonClicked && data && !data.printerProductPOPDFData) {
