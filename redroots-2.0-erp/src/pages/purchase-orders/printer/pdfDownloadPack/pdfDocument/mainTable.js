...
const MainTable = ({ data }) => {
  const grandTotal = data.pack_detail.reduce(
    (acc, obj) =>
      acc +
      obj.fulfilled_quantity *
        obj.unit_cost *
        (1 + obj.cgst / 100 + obj.sgst / 100 + obj.igst / 100),
    0,
  )

  console.log('pack_detail', data.pack_detail)
  ...
  const createTableFooter = () => {
    return (
      <View style={styles.rows}>
        <View style={styles.firstColHeader}>
          <Text style={{ ...styles.cellInHeader, width: 185.2, textAlign: 'center' }}>Total</Text>
        </View>

        <View style={styles.restColHeaders}>
          <Text style={{ ...styles.cellInHeader, width: 260.8, textAlign: 'left' }}>
            {data.pack_detail.reduce((acc, obj) => acc + obj.fulfilled_quantity, 0)}
          </Text>
        </View>
