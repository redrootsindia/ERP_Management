...
const InvoiceHeader = ({ data }) => (
  <View style={{ marginTop: 15 }} fixed>
    <View>
      <Text style={styles.headerTitle}>RTV GRN</Text>
    </View>
    <View style={styles.verticalDivide}>
        ...
        <View style={styles.verticalDivide}>
          <View style={styles.horizontalDivide}>
            <View style={{ ...styles.header, height: 60, width: 143, ...styles.noBorderLeftWidth }}>
              ...
              <Text style={styles.voucherBelow}>{data.name}</Text>
            </View>
            <View
              style={{
                ...styles.header,
            ...
            >
              ...
              <Text style={styles.voucherBelow}>
                {moment(Number(data.received_on)).format('Do MMM YYYY')}
              </Text>
            </View>
          </View>
          <View style={styles.horizontalDivide}>
      ...
      <View style={styles.horizontalDivide}>
        ...
        >
          ...
          <Text style={styles.companyName}>{data.buyer_id}</Text>
          {/* <Text style={styles.companyGst}>
            <div style={{ display: 'inline-block', ...styles.bold }}>GSTIN:</div>
          ...
          <Text style={styles.voucher}>
            <div style={{ display: 'inline-block', ...styles.bold }}>Market Place :</div>
            {data.buyer_data?.marketplace_channel}
          </Text>
          <Text style={styles.voucherBelow}>
            <div style={{ display: 'inline-block' }}>Buyer :</div>
            {data.buyer_data?.name}
          </Text>
          <Text style={styles.voucherBelow}>{data.buyer_data?.phone}</Text>
          <Text style={styles.voucherBelow}>{data.buyer_data?.email}</Text>
        </View>
      </View>
    </View>
  </View>
