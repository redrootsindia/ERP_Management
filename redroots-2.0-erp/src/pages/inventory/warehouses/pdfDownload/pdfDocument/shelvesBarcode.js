...
const ShelvesBarcode = ({ data }) => (
  <>
    <View style={{ textAlign: 'center' }} fixed>
      <Text>{`${data.name} (${data.location})`}</Text>
    </View>
    <View style={styles.table}>
      {data.racks.map((row, ind) => (
        <View key={ind} style={[styles.tableRow]}>
          {row.map((cell, j) => {
            const canvas = document.createElement('canvas')
            JsBarcode(canvas, cell.id, {
              width: 1,
              height: 40,
              displayValue: false,
            })
            ...
            return (
              ...
              >
                ...
                <Text style={{ width: 125, fontSize: 10 }}>
                  {`${cell.rackName} - ${cell.shelvesName}`}
                </Text>
              </View>
            )
          })}
