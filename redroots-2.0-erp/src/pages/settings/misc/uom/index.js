...
import { UOMS } from './queries'

const mapStateToProps = ({ user }) => ({ user })

const UoMs = ({ user: { permissions } }) => {
  const [uoms, setUoMs] = useState([])

  const [uomID, setUOMID] = useState(undefined)
  const [newUOMIDToLoad, setNewUOMIDToLoad] = useState(undefined)
  const [changesMadeInForm, setChangesMadeInForm] = useState(false)
  const [discardModalVisible, setDiscardModalVisible] = useState(false)

  const { loading, error, data, refetch } = useQuery(UOMS)

  useEffect(() => {
    if (data && data.uoms && data.uoms.length) setUoMs(data.uoms)
  }, [data])
                      ...
                      emptyText: (
                        <div className="custom-empty-text-parent">
                          <div className="custom-empty-text-child">
                            ...
                            <h5>No Units of Measurement Found</h5>
                          </div>
                        </div>
                      ),
                    }}
                    onRow={(record) => ({
                      onClick: () => {
                        if (changesMadeInForm) {
                          ...
                          setNewUOMIDToLoad(record.id)
                        }
                        if (!changesMadeInForm && !discardModalVisible) setUOMID(record.id)
                      },
                    })}
                  />
                  <ConfirmDiscard
...
export default withRouter(connect(mapStateToProps)(UoMs))
