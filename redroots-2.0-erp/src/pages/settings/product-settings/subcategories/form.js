...
import { PRODUCT_SUBCAT, UPSERT_PRODUCT_SUBCAT } from './queries'

const { Option } = Select

const ProductSubcatForm = (props) => {
  // prettier-ignore
  ...
  const [upsertProductSubcategory] = useMutation(UPSERT_PRODUCT_SUBCAT)
  const { loading: subcatLoad, error: subcatErr, data: subcatData } = useQuery(PRODUCT_SUBCAT, {
  ...
  useEffect(() => {
    if (subcatData && subcatData.productSubcategory) {
      const { product_category_id } = subcatData.productSubcategory
      if (subcatData.productSubcategory.name) setName(subcatData.productSubcategory.name)
  ...
  const onSubmit = () => {
    ...
    upsertProductSubcategory({
      ...
      .catch((err) => {
        setOkText(subCatID ? 'Save' : 'Create')
        notification.error({
          message: 'Error occured while saving product-subcategory.',
        ...
        <div className="card">
          <div className="card-header">
            <div className="row">
              <div className="col-9">
                <h5 className="mb-2">
                  <strong>{subCatID ? 'Edit' : 'Add'} Product Subcategory</strong>
                </h5>
              </div>
