...
import { EXPENSE_CATS } from './queries'
...
const CategoriesList = ({ user: { permissions } }) => {
  ...
  const { loading, error, data, refetch } = useQuery(EXPENSE_CATS)

  useEffect(() => {
    if (data && data.expenseCategories && data.expenseCategories.length)
  ...
  return (
    <div>
      <Helmet title="Expense Categories" />
