...
import { EXPENSE_CAT, UPSERT_EXPENSE_CAT } from './queries'
...
const CategoryForm = (props) => {
  ...
  const [upsertExpenseCategory] = useMutation(UPSERT_EXPENSE_CAT)
  const { loading: catLoad, error: catErr, data: catData } = useQuery(EXPENSE_CAT, {
    variables: { id: catID },
  })

  useEffect(() => {
    if (catData) {
      if (catData.expenseCategory && catData.expenseCategory.length !== 0) {
        setName(catData.expenseCategory.name)
        setDescription(catData.expenseCategory.description)
      }
    }
  }, [catData])
  ...
  const onSubmit = () => {
    ...
    upsertExpenseCategory({
      variables: { upsertType: catID ? 'update' : 'create', id: catID, name, description },
    })
      .then(() => {
        setOkText(catID ? 'Save' : 'Create')
      ...
      .catch((err) => {
        setOkText(catID ? 'Save' : 'Create')
        notification.error({
          message: 'Error occured while saving Expense-category.',
        ...
        <div className="card">
          <div className="card-header">
            <div className="row">
              <div className="col-9">
                <h5 className="mb-2">
                  <strong>{catID ? 'Edit' : 'Add'} Expense Category</strong>
                </h5>
              </div>
